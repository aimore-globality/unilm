...
def parse_args():
    ...
    parser.add_argument(
        "--suffix", default="", type=str, help="Other suffix to distinguish different dataset."
    )
...
def dataset_split(args, dataset):
    ...
    with open(
        osp.join(args.root_dir, dataset["version"] + "_train_" + args.suffix + ".json"), "w"
    ) as f:
        f.write(json.dumps(train_dataset))
    test_dataset = {"version": dataset["version"], "data": test_data}
    with open(
        osp.join(args.root_dir, dataset["version"] + "_test_" + args.suffix + ".json"), "w"
    ) as f:
        f.write(json.dumps(test_dataset))
    dev_dataset = {"version": dataset["version"], "data": dev_data}
    with open(
        osp.join(args.root_dir, dataset["version"] + "_dev_" + args.suffix + ".json"), "w"
    ) as f:
