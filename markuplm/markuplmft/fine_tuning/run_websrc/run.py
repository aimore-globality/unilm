        ...
        with torch.no_grad():
            inputs = {
                ...
                "token_type_ids": batch[
                    2
                ],  # TODO(aimore): It seems there is a bug here. It should be batch[3], batch[4]
...
def load_and_cache_examples(args, tokenizer, max_depth=50, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            "dev" if evaluate else "train", "markuplm", str(args.max_seq_length), str(max_depth)
        ),
    )
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if output_examples:
            logger.info("read_squad_examples...")
            examples, tag_list = read_squad_examples(
        ...
        examples, _ = read_squad_examples(
            input_file=input_file,
            root_dir=args.root_dir,
            is_training=not evaluate,
            tokenizer=tokenizer,
...
def main():
    ...
    parser.add_argument("--warmup_ratio", default=0.0, type=float, help="RT.")
    parser.add_argument(
        "--n_best_size",
        default=20,
        type=int,
    ...
    parser.add_argument(
        "--save_steps", type=int, default=3000, help="Save checkpoint every X updates steps."
    )
    parser.add_argument(
    ...
    logger.info("Training/evaluation parameters %s", pformat(args))
    ...
    if args.do_train:
        train_dataset = load_and_cache_examples(
            args, tokenizer, max_depth=max_depth, evaluate=False, output_examples=False
        )
