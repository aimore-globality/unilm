...
def parse_args():
    ...
    parser.add_argument(
        "root_dir", metavar="./data", help="The root directory of the raw WebSRC dataset"
    )
...
def compute_pos(f, t_gold, addition, t_pred):
    ...
    if e_gold is None:
        if len(t_pred) != 1:
            ...
            if (
                (e_pred is not None)
                or (addition == 1 and e_prev is not None)
                or (addition == 0 and e_prev is None)
            ):
                return 0
            else:
                return 1
...
def main(opts):
    ...
    for p in pages_list:
        pages_ans_qids = [
            k
            for k, _ in qid_to_has_ans.items()
            if p in k
        ]
