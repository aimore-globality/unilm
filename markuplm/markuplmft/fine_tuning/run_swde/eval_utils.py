...
# def page_hits_level_metric(lines_df):
...
# def site_level_voting(lines_df):
...
#     return page_hits_level_metric(lines_df)


# def page_level_constraint(lines_df):
...
#     # lines_df['predictions']  = lines_df['final_probs'].apply(lambda final_probs: [x for e, x in enumerate(tags) if final_probs[e]>0.5 ]) # TODO (AIMORE): What to do in case there are multiple tags?
...
#     return site_level_voting(lines_df)
