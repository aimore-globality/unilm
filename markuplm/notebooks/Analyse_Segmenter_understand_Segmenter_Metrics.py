...
assert len(classified_df[classified_df['node_text'].apply(lambda x: 'http' in x)]) == 0
assert len(classified_df[classified_df['node_gt_text'].apply(lambda row: any(['http' in x for x in row]))]) == 0
len(classified_df)

# %%
domain_names = ["hawthorneadvertising.com", "workingfamilies.org.uk"]
...
# %%
list(classified_df[classified_df['domain'] == domain_name][classified_df[classified_df['domain'] == domain_name]["node_text"].apply(lambda row: any([True if x in row else False for x in gt_text ]))]['node_text'].values)
...
domain_names = ["hawthorneadvertising.com", "workingfamilies.org.uk", "themplc.co.uk"]
domain_name = domain_names[-1]
print(domain_name)
gt_text = list(fn_error[fn_error['domain'] == domain_name]["FN_seg"].apply(lambda x: x[1]).values)
classified_df[classified_df['domain'] == domain_name][classified_df[classified_df['domain'] == domain_name]["node_text"].apply(lambda row: any([True if x in row else False for x in gt_text ]))]
...
all_classified_df = predicted_df[predicted_df['domain']=='themplc.co.uk'].explode("nodes").reset_index()
...
all_classified_df[all_classified_df['url'] == "https://www.themplc.co.uk/page/rightsholders"][["url", "node_text"]]
